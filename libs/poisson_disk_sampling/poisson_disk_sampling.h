#pragma once
#ifndef OPENGL_POSSION_DISK_SAMPLING
#define OPENGL_POSSION_DISK_SAMPLING
...
auto disks(float kRadius, float size) -> std::vector<std::array<float, 3>> {
	// Input parameters.
	constexpr auto kXMin = std::array<float, 3>{{0, 0,0}};
	auto kXMax = std::array<float, 3>{{size, size,size}};
...
auto disks2(float kRadius, float size) -> std::vector<std::array<float, 2>> {
	// Input parameters.
	constexpr auto kXMin = std::array<float, 2>{{0, 0}};
	auto kXMax = std::array<float, 2>{{size, size}};
